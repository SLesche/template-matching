...
condition_bins = [1]; % We only have one condition in example data
...
electrodes = [1]; % We only have one electrode saved in example data
polarity = ["positive"];
windows = {[250 600]};
...
erp_data = erp_data(:, :, :, condition_bins); % Only fit data for the condition bins that are relevant
[n_subjects, n_chans, n_times, n_bins] = size(erp_data);

% If you can use the Parallel Computing Toolbox
results_mat = run_template_matching(erp_data, time_vec, comb, 1); % Run the third (3) method saved in comb. You can run any 
