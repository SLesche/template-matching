function[] = run_simulation(n_simuls, sliced_trials, time_vec, method_table, db_location)
    ...
    elseif method_table.approach == "peak" || method_table.approach == "area"|| method_table.approach == "liesefeld_area"
    ...
    parfor isimul = 1:n_simuls
        ...
        window_control = [NaN NaN];
        window_simul = [NaN NaN];
        ...
        percent_amp = 0;
        percent_buffer = 0;
        switch method_table.window_name
            case "p3_narrow"
                percent_amp = 0.5;
                percent_buffer = 0;
            case "p3_normal"
        ...
        if method_table.window_name == "p3_oldmethods"
            if method_table.approach == "peak"
        ...
        elseif method_table.window_name == "p3_narrow" || method_table.window_name == "p3_normal" || method_table.window_name == "p3_wide"
            [window_control] = find_windows(ga_control, time_vec, [200 900], 'positive', percent_amp, percent_buffer);
            [window_simul] = find_windows(ga_simul, time_vec, [200 900], 'positive', percent_amp, percent_buffer);
        elseif method_table.window_name == "p3_250_700"
            window_control = [250 700];
            window_simul = [250 700];
        ...
        lat_ga_control = approx_area_latency(time_vec, ga_control, [window_control(1) window_control(2)], polarity, 0.5, true);
            ...
            if is_template_matching
                for isubject = 1:n_subjects
                    ...
                    params = run_multi_start(define_optim_problem(specify_objective_function(time_vec', signal', ga_simul', [window_simul(1) window_simul(2)], polarity, weight_function, eval_function, normalize_function ,penalty_function)));
        ...
        if is_template_matching
            for isubject = 1:n_subjects
                ...
                params = run_multi_start(define_optim_problem(specify_objective_function(time_vec', signal', ga_control', [window_control(1) window_control(2)], polarity, weight_function, eval_function, normalize_function ,penalty_function)));
